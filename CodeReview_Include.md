...
```
 <details>
  <summary>상태 : [미결] , 위험 : 99, 횟수 : 16, 추적 : 2026/04/19 - 2026/04/19</summary>
...
```
 <details>
  <summary>상태 : [미결] , 위험 : 95, 횟수 : 16, 추적 : 2026/04/19 - 2026/04/19</summary>
  <br>- 설명 : `main()` 내에서 `pP`가 선언되지 않은 상태로 `*pP` 역참조가 수행되어 컴파일 에러(undeclared identifier) 발생. 빌드 단계에서 차단됨. 이전 커밋에서 주석으로 비활성화되어 있던 `*pP` 역참조가 활성화된 회귀(regression)
...
```
 <details>
  <summary>상태 : [미결] , 위험 : 90, 횟수 : 16, 추적 : 2026/04/18 - 2026/04/19</summary>
  <br>- 설명 : `p5`가 `TEST()` 스코프 및 전역에 선언되어 있지 않음. `if(false)` 분기는 실행되지 않지만 컴파일 단계에서 반드시 파싱되어 에러 발생. 해당 블록은 영구 데드 코드. Huns02 시점 `*pT`(미선언)에서 `*p5`(미선언)으로 변수명만 변경된 이력 존재
...
```
 <details>
  <summary>상태 : [미결] , 위험 : 90, 횟수 : 16, 추적 : 2026/04/19 - 2026/04/19</summary>
...
```
 <details>
  <summary>상태 : [미결] , 위험 : 85, 횟수 : 16, 추적 : 2026/04/19 - 2026/04/19</summary>
