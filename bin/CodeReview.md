...
* 중요도 = P0:Critical, P1:High, P2:Medium, P3:Low, P4:Minimal(위험도 10점미만 P4보정) <- 이슈 우선 순위

* 이슈매칭 = 키(제목·커밋·해쉬·라인 등) 완전 일치가 아닌, 제목·코드·파일·함수·설명·내용 등 기준 의미적 동일성으로 판단

* 이슈카운트 = 최신{$Output}의 경우 1, {$Include}에 병합 시점에 기존 이슈라면 +1
...
**이슈카운트 등 히스토리가 필요한 경우 {$Include}와 최신{$Output}만 오직 사용({$Include}와 최신{$Output} 외 절대 사용 금지)**
...
**병합은 최신 내용을 기준으로 기존 내용을 참조하여 보완해서 저장**

**요약 및 이슈는 제목 포함 정해진 형식만으로 구성하며 '사유' 제외 모든 항목 기입 필수**

**[해결] 시점에만 기존 사유 내용에 병합(사유 변경 금지)**

**모든 파일에 정해진 형식의 이슈만 작성(이슈 외 작성 금지. {$Output}만 요약 작성 예외)**

**파일에 새로운 이슈를 추가하거나 기존 이슈를 병합할 경우 '중요도 > 위험도'별 맨위에 저장**

**이슈 [해결]은 코드 변경이 없거나 관련 커밋이 없는 경우 해결 금지, 중요도와 위험도를 조정**
...
[중요도:2개의 영단어 요약][작업자 이름만][대표 커밋 해쉬 1개만][파일 이름 1개만/함수면 또는 클래스명 1개만/Line]
짧게 내용 요약
...
printf nullptr 역참조 크래시(7단어 이하 요약)
``` 소스 코드 1줄 이상 필수
