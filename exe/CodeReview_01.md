...
- Execute01_7: 코드 리뷰 결과들 중 [미결] 이슈들만 원인 코드와 이슈 커밋(해쉬/담당자)의 소스 변경점이 일치하는지 다시 확인 → 일치하지 않을 경우 해당 파일의 최근 20개 커밋 소스 변경점 조사 → 그래도 못 찾으면 '해쉬/담당자'를 '000000/미지정'으로 설정

- Execute01_8: 코드 리뷰 결과들 중 [미결] 이슈들만 최신{$Output}을 생성하여 저장하고, 맨위에 요약 갱신 저장
