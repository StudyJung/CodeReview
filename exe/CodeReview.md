...
$Scope = 오늘이 (일,수,토)요일이면 'LastTag이후 ~ HEAD까지 코드 리뷰', 나머지는 '일주일전 ~ HEAD까지 코드 리뷰'

## FUNCTION

* 위험도 = 01~99점까지 위험이 클수록 높은 점수 <- 중요도 상승, 하락 등에 수치 조정({$Include}에서 P0, P1 이슈만)

* 중요도 = P0:Critical, P1:High, P2:Medium(위험도 60점~20점), P3:Low(위험도 40점~10점), P4:Minimal(위험도 20점~1점) <- 이슈 우선 순위 판단하여 위험도 분포와 비슷하게 보정 업데이트
...
1. [중요도:2개의 영단어 요약][커밋 해쉬(대표1개만)/왼쪽 커밋 해쉬 작업자][파일명 1개만/함수명 또는 클래스명 1개만/Line:33-100]
...
6. <details></details>
7. '- 사유 :'는 내용이 없으면 삭제. [미결]에서 [해결]로 변경되었을 경우에만 기존 '사유' 내용에 추가로 해결 내용 기입
...
```
 ...
 - 사유 : 내용이 없으면 '사유' 항목 삭제. 담당자 수동 기입 가능. [미결]에서 [해결]로 변경되었을 경우에만 기존 '사유' 내용에 추가로 해결 내용 기입.
