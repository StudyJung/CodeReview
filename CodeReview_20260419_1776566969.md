# CodeReview_20260419_1776566969.md
- "컴파일되지 않는 코드는 아직 코드가 아니다 — 빨간 줄부터 지워라."

## 요약
| 항목 | 내용 |
| :--- | :--- |
| **Git Range** | Initial(c430964) - HEAD(7e48afd) ( Commits 3 / 소스변경 2 ) |
| **Period** | 2026/04/19 11:34:26 - 2026/04/19 11:47:31 |
| **Model(Effort)** | claude-sonnet-4-6 (--effort high) |
| **Tools** | '/check' |

| 등급 | 건수 | 설명 |
| :--- | :--- | :--- |
| **P0:Critical** | 5개 | nullptr 역참조 1건, 미선언 식별자 2건, 미존재 헤더 2건 |
| P1:High | 0개 | |
...
```
 <details>
  ...
  <br>- 설명 : `main()` 내에서 `pP`가 선언되지 않은 상태로 `*pP` 역참조가 수행되어 컴파일 에러(undeclared identifier) 발생. 빌드 단계에서 차단됨
...
```
 <details>
  ...
  <br>- 설명 : `p5`가 `TEST()` 스코프 및 전역에 선언되어 있지 않음. `if(false)` 분기는 실행되지 않지만 컴파일 단계에서 반드시 파싱되어 에러 발생. 해당 블록은 영구 데드 코드
...
```
 <details>
  <summary>상태 : [미결] , 위험 : 90, 횟수 : 1, 추적 : 2026/04/19 - 2026/04/19</summary>
  <br>- 설명 : `Huns.h`가 저장소에 존재하지 않음. 이 파일을 포함하는 모든 TU가 빌드 불가. `printf` 사용을 위해 `<cstdio>` 등 표준 헤더도 누락된 상태
  <br><br>- 의견 : `"Test.h"`로 변경하거나, `Huns.h`를 생성하여 필요한 선언을 포함. `printf` 사용을 위해 `<cstdio>` 포함 필요
